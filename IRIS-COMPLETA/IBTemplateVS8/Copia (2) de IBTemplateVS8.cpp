...
int main(int argc, _TCHAR* argv[])
{
  ...
  N = cvRound(rs);
  //rows = N, cols = M
  angular = computeAngular(src_gray, rp, rs, xc, yc, M, N);
  normal = computeNorm(src_gray, angular, rp, rs, xc, yc, M, N);
  
  /// Show your results
  convertScaleAbs(src, src);
  convertScaleAbs(angular, angular);
  convertScaleAbs(normal, normal);
  
  namedWindow("Original", CV_WINDOW_AUTOSIZE);
  imshow( "Original", src);
  imshow( "Sin Normalizar", angular);
  imshow( "Normalizada", normal);
